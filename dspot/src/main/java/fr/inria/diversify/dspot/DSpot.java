...
/**
 * User: Simon Date: 08/06/15 Time: 17:36
 */
public class DSpot {
	...
	public DSpot(InputConfiguration inputConfiguration) throws InvalidSdkException, Exception {
		this(inputConfiguration, 3,
				Arrays.asList(new TestDataMutator(), new TestMethodCallAdder(), new TestMethodCallRemover(),
						new StatementAdderOnAssert(), new StatementAdd(inputConfiguration.getProperty("filter"))),
	...
	public DSpot(InputConfiguration configuration, int numberOfIterations) throws InvalidSdkException, Exception {
		this(configuration, numberOfIterations,
				Arrays.asList(new TestDataMutator(), new TestMethodCallAdder(), new TestMethodCallRemover(),
						new StatementAdderOnAssert(), new StatementAdd(configuration.getProperty("filter"))));
	}
	...
	public DSpot(InputConfiguration inputConfiguration, int numberOfIterations, List<Amplifier> amplifiers)
			throws InvalidSdkException, Exception {
		this(inputConfiguration, numberOfIterations, amplifiers, new BranchCoverageTestSelector(10));
	}

	public DSpot(InputConfiguration inputConfiguration, int numberOfIterations, List<Amplifier> amplifiers,
			TestSelector testSelector) throws InvalidSdkException, Exception {
		...
		File tmpDir = new File(inputConfiguration.getProperty("tmpDir")+"/tmp");
		if (!tmpDir.exists()) {
			tmpDir.mkdir();
		} else {
...
//				new File(inputConfiguration.getProperty("tmpDir") + "/tmp"));
		...
		// We need to use separate factory here, because the BranchProcessor will
		// process test also
		...
		final File projectJsonFile = new File(
				this.inputConfiguration.getOutputDirectory() + "/" + splittedPath[splittedPath.length - 1] + ".json");
		...
		try {
			...
			if (resources != null) {
				this.testResources.addAll(
						Arrays.stream(resources).map(this::relativePathFromListFile).collect(Collectors.toList()));
				FileUtils.copyDirectory(resourcesDirectory, new File(path));
			}
	...
	public List<CtType> amplifyAllTests() throws InterruptedException, IOException, ClassNotFoundException {
		final List<CtType> amplifiedTest = inputProgram.getFactory().Class().getAll().stream()
				...
				.filter(ctClass -> ctClass.getMethods().stream().anyMatch(AmplificationChecker::isTest))
				.map(this::amplifyTest).collect(Collectors.toList());
		writeTimeJson();
	...
	private void writeTimeJson() {
		...
		final File file = new File(
				this.inputConfiguration.getOutputDirectory() + "/" + this.projectTimeJSON.projectName + ".json");
	...
	public CtType amplifyTest(String fullName, List<String> methods)
			throws InterruptedException, IOException, ClassNotFoundException {
		CtType<Object> clone = this.compiler.getLauncher().getFactory().Type().get(fullName).clone();
		clone.setParent(this.compiler.getLauncher().getFactory().Type().get(fullName).getParent());
		return amplifyTest(clone, methods.stream().map(methodName -> clone.getMethodsByName(methodName).get(0))
	...
	public CtType amplifyTest(CtType test, List<CtMethod<?>> methods) {
		try {
			Counter.reset();
			Amplification testAmplification = new Amplification(this.inputConfiguration, this.amplifiers,
					this.testSelector, this.compiler);
			...
			CtType<?> amplification = AmplificationHelper.createAmplifiedTest(testSelector.getAmplifiedTestCases(),
					test);
	...
	public void cleanResources() {
		if (this.testResources != null) {
			this.testResources.stream().map(File::new).filter(File::exists).forEach(file -> {
				try {
